...
const allowedFields = [
  'id_product', 'name', 'category', 'price', 'salePrice', 'discount',
  'description', 'details', 'rating', 'reviews',
...
const prepareProductData = (body: any, images: string[] = []) => {
  ...
  allowedFields.forEach(field => {
    if (body[field] !== undefined) {
      if (['price', 'salePrice', 'discount', 'rating', 'reviews'].includes(field)) {
        ...
        if (!Array.isArray(detailArr)) throw new Error('Details must be an array');
        ...
        if (!Array.isArray(sizes)) throw new Error('availableSizes must be an array');
...
export const getProductById = async (req: Request, res: Response, _next: NextFunction): Promise<void> => {
  ...
  try {
    const product = await Product.findOne({ id_product: id }).lean({ virtuals: true });
    if (!product) {
