...
export function SortableImage({ id, file, index, onRemove, onPreview }: Props) {
  const {
    attributes,
    listeners,
    setNodeRef,
    ...
    id,
    animateLayoutChanges: () => true, // ทำให้ drag ลื่นขึ้น
  });
...
return (
    ...
    >
        ...
        src={imageUrl}
        alt={`Image ${index + 1}`}
        onClick={() => onPreview(imageUrl)} // ✅ ส่ง URL จริง ไม่ส่ง File
